...
class FinalMLP(BaseModel):
    def __init__(self, 
        ...
        self.mlp1 = FactorizationMachine(feature_map=feature_map)
        self.mlp2 = FactorizationMachine(feature_map=feature_map)
        #self.fcl = nn.Linear(400, 1)
        self.use_fs = use_fs
        if self.use_fs:
    ...
    def forward(self, inputs):
        ...
        y_pred = self.fusion_module(self.mlp1(X, feat1), self.mlp2(X, feat2))
    
        #y_pred = self.mlp1(flat_emb)
        #y_pred = self.fcl(y_pred) 
